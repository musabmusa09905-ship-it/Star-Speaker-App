...
-- Allow manual bulk reminder and encouragement marks from the Daily Reminder Dashboard.
...
-- Keep the same privacy-preserving board rules, but count writing submissions
-- as output-habit submissions alongside speaking submissions.
drop function if exists public.get_consistency_board();

create function public.get_consistency_board()
returns table (
  period text,
  student_id uuid,
  display_name text,
...
as $$
  ...
  current_user_context as (
    select public.get_current_user_role() as role
  ),
  assigned_to_current_teacher as (
  ...
  safe_submissions as (
    ...
    select
      ...
      ws.student_id,
      ws.submitted_at,
      ws.submitted_at::date as submitted_day,
      nullif(trim(coalesce(ws.self_reflection, '')), '') is not null as has_reflection,
      wt.due_date
    from public.writing_submissions ws
    left join public.writing_tasks wt on wt.id = ws.task_id
    where ws.submitted_at is not null
      and coalesce(ws.status, 'submitted') in ('submitted', 'reviewed')
  ),
  student_days as (
  ...
  ranked as (
    select
      ...
      exists (
        select 1
        from assigned_to_current_teacher assigned
        where assigned.student_id = pp.student_id
      ) as is_assigned_student,
      sb.badges
  ...
  from ranked
  cross join current_user_context cuc
  where auth.uid() is not null
    and (
      cuc.role = 'admin'
      or (
        cuc.role = 'teacher'
        and (ranked.rank_position <= 10 or ranked.is_assigned_student)
      )
      or (
        cuc.role = 'student'
        and (ranked.rank_position <= 10 or ranked.is_current_user)
      )
    )
  order by
    case ranked.period
